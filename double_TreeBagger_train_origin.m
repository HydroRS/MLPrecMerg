function [ Performance,y,y_vali,y_vali01,y_class,y_vali_class,y_vali01_class,Weights ] = double_TreeBagger_train_origin( start_day,end_day,...
            train_period,In,Out,Out_regression, In_vali,Out_vali, In_vali01,Out_vali01,...
            leaf,ntrees, fboot,surrogate,paroptions,train_data,validation_data,validation_data01,pcp_threshold)
            ...
            for month = 1:end_month
                ...
                for  month_day =1:num_day
                   ...
                   temp_out_regression=Out_regression(current_day+1:(current_day+length(train_data)*1),:);
                    ...
                    % train
                   y_temp=predict(b, temp_in);
                    y_temp= str2num(char(y_temp)) ;
                    y_class=[y_class;y_temp];
                    % subtest 01
                    y_temp_vali=predict(b, temp_in_vali); % classify
                     y_temp_vali= str2num(char(y_temp_vali));% cell to mat
                     y_vali_class=[y_vali_class;y_temp_vali];
                       % subtest 02
                      y_temp_vali01=predict(b, temp_in_vali01); % classify
                     y_temp_vali01= str2num(char(y_temp_vali01));% cell to mat
                    y_vali01_class=[y_vali01_class;y_temp_vali01];
