function [ Performance,y,y_vali,y_vali01 ] = ELM_train( start_day,end_day,...
            train_period,In,Out,Out_regression,In_vali,Out_vali,In_vali01,Out_vali01,train_data,...
            validation_data,validation_data01,pcp_threshold,hiddenSizes, radon_run)
...
%                        model = elm_train_base(temp_out_regression,temp_in,0,20,'sig');
%                        
%                       y_temp_ = elm_predict(temp_in,model);
%                       y_temp_vali_ = elm_predict(temp_in_vali,model);
% 					  y_temp_vali_01 = elm_predict(temp_in_vali01,model);
                      
                  
                     
%                       
                      net_best = ELM_optimization( hiddenSizes, radon_run,temp_in, temp_out_regression );
                    y_temp_all=[];
                   y_temp_vali_all=[];
                   y_temp_vali_all02=[];
                    [m,n]=size(net_best);
                   for mm=1:m*n
                    y_temp_01 = elm_predict(temp_in,net_best{mm});%,'UseParallel','yes');
                     y_temp_all=[y_temp_all,y_temp_01'];
                     
                   y_temp_vali_01=elm_predict(temp_in_vali,net_best{mm});
                  y_temp_vali_all=[y_temp_vali_all,y_temp_vali_01'];
                  
                   y_temp_vali_02=elm_predict(temp_in_vali01,net_best{mm});
                  y_temp_vali_all02=[y_temp_vali_all02,y_temp_vali_02'];
               
                   end
                    
                    y_temp_=mean(y_temp_all,2);
                   y_temp_vali_01=mean(y_temp_vali_all,2); 
                     y_temp_vali_02=mean(y_temp_vali_all02,2);
%                          反归一化
                     y_temp_=mapminmax('reverse',y_temp_,PS02);
                      y_temp_vali_=mapminmax('reverse',y_temp_vali_01,PS02);
					  y_temp_vali_01=mapminmax('reverse',y_temp_vali_02,PS02);
%                       
%                        y_temp_=mapminmax('reverse',y_temp_,PS02);
%                       y_temp_vali_=mapminmax('reverse',y_temp_vali_,PS02);
