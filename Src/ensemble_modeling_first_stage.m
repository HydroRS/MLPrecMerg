function [ Prediction_sub_test01_vali_new,Prediction_sub_test02_vali_new ] = ensemble_modeling_first_stage( start_day, end_day,validation_subset01,...
    validation_subset02,Out_vali,Prediction_sub_test01_vali,Out_vali01,...
    Prediction_sub_test02_vali,In_tain,Out_train_regression,train_data,Ensemble_method)
...
for year = start_day(1):end_day(1)
    ...
    for month = 1:end_month
        ...
        for  month_day =1:num_day
            ...
            temp_out_vali=Out_train_regression(current_day_train+1:(current_day_train+length(train_data)*1),:); % outvali is observations
            temp_predict = In_tain(current_day_train+1:(current_day_train+length(train_data)*1),:); % predicitions by the 3 ML methods
            
            temp_out_vali_test01=Out_vali(current_day_vali+1:(current_day_vali+length(validation_subset01)*1),:); % outvali is observations
            temp_predict_test01 = Prediction_sub_test01_vali(current_day_vali+1:(current_day_vali+length(validation_subset01)*1),:); % predicitions by the 3 ML methods
            ...
            elseif Ensemble_method==2 % Inverse Weight Error Weight
                ...
                [m1,n1]=size(temp_predict_test01);
                output=sum(repmat(weight,m1,1).*temp_predict_test01,2);
            ...
            elseif Ensemble_method==3 %nuging method
                ...
                [R2, RMSE, NSE, output ] = nudging_Integration (temp_predict_test01, temp_out_vali_test01, Optimaized_value );
            ...
            elseif  Ensemble_method==4 % Outlier average
                ...
                [m,n]=size(temp_predict_test01);
                id_repmat=repmat(id,m,1);
                output=mean(id_repmat.*temp_predict_test01,2)*n/(n-1);
          ...
          current_day_train=current_day_train+1*length(train_data);
...
Prediction_sub_test01_vali_new(Prediction_sub_test01_vali_new<0)=0;
Prediction_sub_test02_vali_new(Prediction_sub_test02_vali_new<0)=0;
